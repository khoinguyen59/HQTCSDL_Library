...
export async function getAllBookSumDB(context) {
  console.log(context);
  let query =
    "SELECT B.ISBN, INITCAP(B.TITLE) AS TITLE, B.IMAGE, B.NUMBER_OF_PAGES AS PAGE, B.LANGUAGE, " +
...
export async function updateHistoryDB(context) {
  let query = runProcedure(`EDIT_HISTORY(:USER_ID, :RENT_HISTORY_ID, :PAY)`);
  const binds = {
    USER_ID: context.USER_ID,
    RENT_HISTORY_ID: context.RENT_HISTORY_ID,
    PAY: context.PAY ? 1 : 0,
  };
  console.log(context);
  let result = null;
